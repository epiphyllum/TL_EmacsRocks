...
#+BEGIN_SRC emacs-lisp
    (add-to-list YOUR_NAME_HERE/packages 'monokai-theme)
#+END_SRC
...
- 优酷： http://v.youku.com/v_show/id_XMTU0NjMyNDg5Ng==.html
- Youtube: https://www.youtube.com/watch?v=BpjwzSB2d00&feature=youtu.be

照理我们先修复一些现在存在的小问题。首先是自动配对的小问题，在 Lisp 中我们有时候只需要一个 ='= 但是 Emacs 很好心的帮我们做了补全，但这并不是我们需要的。我们可以通过下面的代码来让使 Lisp 在 Emacs 中的编辑变得更方便（可以将其添加至 =init-default.el= 配置文件中）。
...
还有一个小问题就是解决在不同系统中的换行符，例如在 DOS 系统下的 =\M= 换行符，这让我们有时候在 Unix 系统中很是头疼，因为它的存在会使版本控制误以为整行的代码都被修改过而造成不必要的麻烦。（你可以在[[http://unix.stackexchange.com/questions/32001/what-is-m-and-how-do-i-get-rid-of-it][这里]]找到更多关于 =\M= 的信息）
...
首先我们需要安装它，照理我们需要将其添加至我们的插件列表中去。
...
下面的代码用于配置 Occur Mode 使其默认搜索当前光标下的字符串，
...
我们可以使用 Customized-group 来更改其高亮的背景色，将 =hightlight= 改为 =region= 。

** Org 导出

使用 =C-x C-e= 可以将 Org-mode 文档导出为你需要的格式，例如 HTML 或者 PDF 文件。你现在看到的这本教程就是由 Org-mode 所导出生产的。
...
- Youtube: https://www.youtube.com/watch?v=WIzWxTc1KUI&featur
...
Org-pomodoro 是一个番茄时间工作法的插件（更多关于这个工作法的信息可以在[[https://en.wikipedia.org/wiki/Pomodoro_Technique][这里]]找到）。它的 GitHub 地址在[[https://github.com/lolownia/org-pomodoro][这里]]。在 =(require 'org-pomodoro)= 后可以通过 Customiz-group 来对其进行设置，包括不同休息种类的时常。
